...
class permute_channels(nn.Module):
    ...
    def forward(self, x):
        return torch.permute(x, (self.B, self.C, self.H, self.W)).contiguous()
...
class CVM_Slice_Loc(nn.Module):
    def __init__(self, device):
        super(CVM_Slice_Loc, self).__init__()
        self.device = device
    ...
    def forward(self, grd, sat):
        grd_feature_volume = self.grd_efficientnet.extract_features(grd)  # [1280, 16, 16], 使用CNNs提取特征
        # 将提取出来的特征一维扁平化：[B, D]
        grd_descriptor1 = self.grd_feature_to_descriptor1(grd_feature_volume)  # length 256
        grd_descriptor2 = self.grd_feature_to_descriptor2(grd_feature_volume)  # length 128
        grd_descriptor3 = self.grd_feature_to_descriptor3(grd_feature_volume)  # length 64
        grd_descriptor4 = self.grd_feature_to_descriptor4(grd_feature_volume)  # length 32
        grd_descriptor5 = self.grd_feature_to_descriptor5(grd_feature_volume)  # length 16
        grd_descriptor6 = self.grd_feature_to_descriptor6(grd_feature_volume)  # length 16
        # 将一维特征进行扩展：[B, D, R1, C1]，相当于将地面影像重复处理
        ...
        for i in range(16):
            ...
            matching_score = torch.sum((grd_descriptor_map1 * sat_descriptor_map_window), dim=1, keepdim=True) / (
                        sat_map_norm * grd_map_norm)  # cosine similarity 通过描述子件相似性，得出匹配得分
            if i == 0:
        ...
        score_16 = matching_score_max.squeeze(dim=1)
        score_16_f = torch.flatten(matching_score_max, start_dim=1)
        heatmap_16 = torch.reshape(nn.Softmax(dim=-1)(score_16_f), score_16.size())
        # return heatmap_16
        ...
        for i in range(16):
            sat_descriptor_map_rolled = torch.roll(x, shifts=-i * 16, dims=1)  # 对特征进行旋转，以最相似获取定位输出
            sat_descriptor_map_window = sat_descriptor_map_rolled[:, :grd_des_len, :, :]
            sat_map_norm = torch.norm(sat_descriptor_map_window, p='fro', dim=1, keepdim=True)
        ...
        return (logits_flattened, heatmap, x_ori, matching_score_stacked,
                matching_score_stacked2, matching_score_stacked3, matching_score_stacked4,
                matching_score_stacked5, matching_score_stacked6)
